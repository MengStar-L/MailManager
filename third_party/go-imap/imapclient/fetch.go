...
func writeFetchItems(enc *imapwire.Encoder, numKind imapwire.NumKind, options *imap.FetchOptions) {
	...
	m := map[string]bool{
		...
		"X-GM-MSGID":    options.GmailMessageID,
	}
	for k, req := range m {
		if req {
			listEnc.Item().Atom(k)
...
var (
	...
	_ FetchItemData = FetchItemDataGmailMessageID{}
...
// FetchItemDataGmailMessageID holds data returned by FETCH X-GM-MSGID.
//
// This requires the X-GM-EXT-1 extension.
type FetchItemDataGmailMessageID struct {
	GmailMessageID uint64
}

func (FetchItemDataGmailMessageID) fetchItemData() {}
...
type FetchMessageBuffer struct {
	...
	RFC822Size        int64
	UID               imap.UID
	GmailMessageID    uint64 // requires X-GM-EXT-1
	BodyStructure     imap.BodyStructure
...
func (buf *FetchMessageBuffer) populateItemData(item FetchItemData) error {
	...
	case FetchItemDataUID:
		buf.UID = item.UID
	case FetchItemDataGmailMessageID:
		buf.GmailMessageID = item.GmailMessageID
...
func (c *Client) handleFetch(seqNum uint32) error {
	...
	return dec.ExpectList(func() error {
		...
		case "X-GM-MSGID":
			...
			if !dec.ExpectSP() || !dec.ExpectModSeq(&gmailMessageID) {
				return dec.Err()
			}

			item = FetchItemDataGmailMessageID{GmailMessageID: gmailMessageID}
		case "BODY", "BINARY":
			if dec.Special('[') {
